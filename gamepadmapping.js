(function(scope) {
  ...
  req.onload = function() {
    if (req.status == 200) {
      loadMappings(JSON.parse(atob((req.responseType == "json" ? req.response : JSON.parse(req.responseText))["content"])).mappings);
    }
  };
  req.open("GET", "https://api.github.com/repos/luser/gamepad-data/contents/mappings.json", true);
  req.send(null);
})(window);
